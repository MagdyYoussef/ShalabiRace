...
CMD2Model::CMD2Model( void )
{
	...
	m_vertixList = new Vector3[MAX_MD2_VERTS];
...
{
	m_texid = Load_Texture( filename );

	return (m_texid != Load_Texture( "default" ));
}
...
void CMD2Model::RenderFrame( void )
{
	...
	while( int i = *(ptricmds++) )
	{
		...
		for( /* nothing */; i > 0; i--, ptricmds += 3 )
		{
			...
			// draw the vertex
			glVertex3f( vertlist[ ptricmds[2] ][0], vertlist[ ptricmds[2] ][1], vertlist[ ptricmds[2] ][2] );
			
			m_vertixList[m_numberOfVertices].x = vertlist[ ptricmds[2] ][0];
			m_vertixList[m_numberOfVertices].y = vertlist[ ptricmds[2] ][1];
			m_vertixList[m_numberOfVertices].z = vertlist[ ptricmds[2] ][2];
			m_numberOfVertices++;
		}
