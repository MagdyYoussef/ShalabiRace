...
bool CLoadMD2::ImportMD2(t3DModel *pModel, const char *strFileName, const char *strTexture)
{
	WCHAR strMessage[255] = {0};
	...
	if(!m_FilePointer) 
	{
		...
		wprintf(strMessage, L"Unable to find the file: %s!", strFileName);
	...
	if(m_Header.version != 8)
	{
		...
		wprintf(strMessage, "Invalid file format (Version not 8): %s!", strFileName);
		MessageBox(NULL, strMessage, L"Error", MB_OK);
		return false;
	}
...
void CLoadMD2::ConvertDataStructures(t3DModel *pModel)
{
	...
	pModel->pObjects.push_back(currentFrame);
}
...
void CLoadMD2::ComputeNormals(t3DModel *pModel)
{
	...
	for(int index = 0; index < pModel->numOfObjects; index++)
	{
		...
		t3DObject *pObjects = &(pModel->pObjects[index]);

		// Here we allocate all the memory we need to calculate the normals
		Vector3 *pNormals		= new Vector3 [pObjects->numOfFaces];
		Vector3 *pTempNormals	= new Vector3 [pObjects->numOfFaces];
		pObjects->pNormals		= new Vector3 [pObjects->numOfVerts];

		// Go though all of the faces of this object
		for(int i=0; i < pObjects->numOfFaces; i++)
		{												
			// To cut down LARGE code, we extract the 3 points of this face
			vPoly[0] = pObjects->pVerts[pObjects->pFaces[i].vertIndex[0]];
			vPoly[1] = pObjects->pVerts[pObjects->pFaces[i].vertIndex[1]];
			vPoly[2] = pObjects->pVerts[pObjects->pFaces[i].vertIndex[2]];
		...
		Vector3 vSum;
		vSum.set(0.0, 0.0, 0.0);
		Vector3 vZero = vSum;
		int shared=0;

		for (int i = 0; i < pObjects->numOfVerts; i++)			// Go through all of the vertices
		{
			for (int j = 0; j < pObjects->numOfFaces; j++)	// Go through all of the triangles
			{												// Check if the vertex is shared by another face
				if (pObjects->pFaces[j].vertIndex[0] == i || 
					pObjects->pFaces[j].vertIndex[1] == i || 
					pObjects->pFaces[j].vertIndex[2] == i)
				{
			...
			pObjects->pNormals[i] = DivideVectorByScaler(vSum, float(-shared));

			// Normalize the normal for the final vertex normal
			pObjects->pNormals[i] = Normalize(pObjects->pNormals[i]);	
